...
<div className='flex h-full bg-acontent'>
        ...
        <div className='px-5 w-full h-auto min-h-screen mx-5 mt-2  py-8 font-poppins flex flex-col         place-content-start place-items-center bg-white shadow-xl rounded-xl'>
            <div className='flex place-content-between place-items-center w-full h-14'>
                ...
                <div>
                    {SingleCourse.is_requested &&
                        <>
                        { !SingleCourse.is_active && SingleCourse.is_requested ?
                        <button onClick={() => CourseApprove(course_id.id)} className='bg-green-600 rounded-lg text-center px-5 py-2 text-white text-light'>Approve</button>
                        :
                        <button onClick={() => CourseApprove(course_id.id)} className='bg-red-600 rounded-lg text-center px-5 py-2 text-white text-light'>Unlist</button>
                        }
                        { SingleCourse.is_active?
                        null
                        :
                        <button onClick={() => CourseReject(course_id.id)} className='bg-red-600 rounded-lg text-center px-5 ml-2 py-2 text-white text-light'>Reject</button>
                        }
                        </>
                    }
