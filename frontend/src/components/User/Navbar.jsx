  ...
  return (
    <div className='w-100 h-20 flex font-poppins px-5 p-4 place-items-center place-content-center gap-5'>
      <div className="flex flex-1 place-items-center place-content-start gap-12">
          <h1 className='font-semibold text-2xl text-white'>ELearn.</h1>
          <div className="flex px-10 rounded-3xl border-2 border-white py-2 place-items-center ms-3">
            <AiOutlineSearch className='text-white' size={20}></AiOutlineSearch>
            <input type="text" className='focus:outline-none ms-2 bg-transparent placeholder:text-white' placeholder='search for courses' onChange={(e)=>dispatch(updatesearchdata(e.target.value))} onKeyPress={(e)=>{searchCourse(e)}}/>
          </div>
      </div>
      
        <div className='flex gap-3 place-items-center'>
        
            <Link to="/"><li className='px-1 list-none text-white'>Home</li></Link>
            {user_auth ? (
              <Link to="/mylearning"><li className='px-1 list-none text-white'>My Learning</li></Link>
            )
            :
              null
            }
            <Link to="/course"><li className='px-1 list-none text-white'>Courses</li></Link>
            <Link to="/cart"><BsCart3 className="cursor-pointer text-white"></BsCart3></Link>
        </div> 
        {user_auth ? (
          user_name.is_staff ? (
            <Link to="/InstructHome"><li className='px-1 list-none text-white'>Instructor</li></Link>
          ) : (
            <Link to="/Onboard"><li className='px-1 list-none text-white'>Become an Instructor</li></Link>
          )
        ) : null}
        ...
        <div>
            <div className='flex gap-2'>
            <Link to="/login"><li className='px-1 list-none'></li><button className='px-4 py-2 bg-red-400 mx-2 text-white shadow-xl rounded-xl' >Login</button></Link>
            </div>   
        </div>
        }
